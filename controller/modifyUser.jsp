...
<%
    ...
    if(modifyType.equals("Insert")){
        ...
        if(name.equals(""))
            ...
            if(name.equals(""))
                response.sendRedirect("../mnguser/modify.jsp?err=1&userId="+userId);   
            else if(email.equals(""))
                response.sendRedirect("../mnguser/modify.jsp?err=2&userId="+userId);   
            else if(email.length() < 4)
                response.sendRedirect("../mnguser/modify.jsp?err=3&userId="+userId);   
            else if(!email.matches("[a-z0-9]+@[a-z0-9]+\\.[a-z0-9]+"))
                response.sendRedirect("../mnguser/modify.jsp?err=4&userId="+userId);
            else if(gender == null)
                response.sendRedirect("../mnguser/modify.jsp?err=5&userId="+userId);      
            else if(pass.equals(""))
                response.sendRedirect("../mnguser/modify.jsp?err=6&userId="+userId);
            else{
                try{
